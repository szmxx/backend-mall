<template>
  ...
  >
    ...
    <common-dialog v-model="current.visible" title="设置排序">
      <common-input v-model="current.sort"></common-input>
    </common-dialog>
  </div>
</template>

<script setup lang="ts">
import { TypeList, ColumnList, FormList, StatusMap } from './config'
...
function onEditSort(row: Record<string, unknown>) {
  current.value.sort = row.sort as string
}

function onRemove(row: Record<string, unknown>) {
  console.log(row)
}
</script>
